...
public class ImgValidateCodeUtil {
    ...
    public static Map<String, String> getImgCodeBaseCode(int length) {
        ...
        randomCode = StringUtil.toLowerCase(randomCode);
        System.out.println("验证码是：" + randomCode);
        g.dispose();
        result.put(Constants.Session.IMG_CODE, randomCode);
        String base64Code = "";
        try {
            //返回 base64
        ...
        result.put(Constants.Session.IMG_CODE_DATA, "data:image/png;base64," + base64Code);
        result.put(Constants.Session.IMG_CODE_EFFFFECT_TIME, sdf.format(DateUtil.getNowDate()));
        return result;
    }
}
